...
NUM_EXAMPLES_PER_EPOCH_FOR_TEST = 64
...
#卷积层，调用Utils的初始化函数_variable_with_weight_decay
...
def test(FLAGS):
    max_steps = FLAGS.max_steps #20000
    batch_size = FLAGS.batch_size #5
    train_dir = FLAGS.log_dir # /tmp3/first350/TensorFlow/Logs
    test_dir = FLAGS.test_dir # 'D:/DATA/voc2012/VOC2012/ImageSets/Segmentation/train.txt'
    ...
    with tf.Session() as sess:
        ...
        saver=tf.train.import_meta_graph("C:/Users/w03798/Desktop/Logs/model.ckpt-9000.meta")
        #saver.restore(sess, tf.train.latest_checkpoint("./save")) #使用模型，参数和之前的代码保持一致
        saver.restore(sess, "C:/Users/w03798/Desktop/Logs/model.ckpt-9000")
        # Load checkpoint
...
def training(FLAGS, is_finetune=False):
    ...
    #val_image_filenames, val_label_filenames = get_filename_list(val_dir)#准备图像和label
    
    # For CamVid

    with tf.Graph().as_default():
        train_data_node = tf.placeholder( tf.float32, shape=[batch_size, image_h, image_w, image_c], name='img')#输入占位符
        train_labels_node = tf.placeholder(tf.int64, shape=[batch_size, image_h, image_w, 1], name='label')#输出占位符
        ...
        summary_op = tf.summary.merge_all()#将所有的summary全部保存到磁盘
        ...
        with tf.Session() as sess:
            ...
            summary_writer = tf.summary.FileWriter(train_dir, sess.graph)#指定一个文件用来保存图
            average_pl = tf.placeholder(tf.float32)
            acc_pl = tf.placeholder(tf.float32)
            iu_pl = tf.placeholder(tf.float32)
            average_summary = tf.summary.scalar("test_average_loss", average_pl)#用来显示标量信息
            acc_summary = tf.summary.scalar("test_accuracy", acc_pl)
            iu_summary = tf.summary.scalar("Mean_IU", iu_pl)
                        
            for step in range(startstep, startstep + max_steps,1):
                ...
                if step % 100 == 0:
                    ...
                    total_val_loss = 0.0
                    hist = np.zeros((NUM_CLASSES, NUM_CLASSES))
                    for test_step in range(int(TEST_ITER)):#循环获取loss和预测值
                        #val_images_batch, val_labels_batch = sess.run([val_images, val_labels])
                        val_images_batch, val_labels_batch = sess.run([images, labels])
