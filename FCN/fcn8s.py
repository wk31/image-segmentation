...
class FCN8s(tf.keras.Model):
    def __init__(self, n_class=21):
        ...
        # conv5
        self.conv5_1 = tf.keras.layers.Conv2D(256, 3, activation='relu', padding='same')
        self.conv5_2 = tf.keras.layers.Conv2D(256, 3, activation='relu', padding='same')
        self.conv5_3 = tf.keras.layers.Conv2D(256, 3, activation='relu', padding='same')
        self.pool5   = tf.keras.layers.MaxPooling2D(2, strides=2, padding='same') # 1/32

        # fc6
        self.fc6 = tf.keras.layers.Conv2D(1024, 7, activation='relu', padding='valid')
        self.drop6 = tf.keras.layers.Dropout(0.5)

        # fc7
        self.fc7 = tf.keras.layers.Conv2D(1024, 1, activation='relu', padding='valid')
